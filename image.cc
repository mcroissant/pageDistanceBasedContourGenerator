...
//    namespace std {using namespace __gnu_cxx;}
    ...
    void Image::initialize_greyscale_image(){
        ...
        if(this->image.channels()>1){
            //LOG4CXX_DEBUG(this->logger,"<<CREATING>> " << this->image.rows << " x " << this->image.cols);
            //this->greyscale_image.release();
    ...
    void Image::initialize_binary_image_matrixes(){
        ...
        if (this->image_loaded){//BINARIZE IMAGE - 0 is black , 255 is white
            this->binary_image.release();
            this->binary_image = cv::Mat(this->greyscale_image.rows,this->greyscale_image.cols,CV_8U,cv::Scalar(0,0,0));
            cv::Mat tmp(this->greyscale_image.rows,this->greyscale_image.cols,CV_8U,cv::Scalar(0,0,0));
            ...
            this->binary_integral_image.release();
            this->binary_integral_image = cv::Mat(this->greyscale_image.rows,this->greyscale_image.cols,CV_8U,cv::Scalar(0,0,0));
    ...
    void Image::draw_line(cv::Point2d from, cv::Point2d to,cv::Scalar color){
        cv::line(this->image, from, to,color,3);
    }
