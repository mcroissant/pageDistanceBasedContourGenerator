...
namespace prhlt {
	...
	Line_Histogram::Line_Histogram(cv::Mat &ex_image, cv::Rect rec, int ex_threshold){
        ...
        cv::namedWindow("line_histogram", CV_WINDOW_AUTOSIZE);
        cv::imshow("line_histogram",resized);
        cv::waitKey(-1);*/
		this->binarization_threshold = 127;
		//this->image = cv::Mat(ex_image.rows,ex_image.cols,CV_8U,0);
		if(ex_image.channels()>1){
...
*/
  ...
  void Line_Histogram::calculate(cv::Mat ex_angles_mat, string orientation_mode, string area_mode, int smoothing_factor,int derivate_window_size){
    //ANGLED VERSION
    calculation_initialization(orientation_mode,area_mode);
    this->image = cv::Mat(this->image,this->interest_zone);
    this->angles = cv::Mat(ex_angles_mat,this->interest_zone);
    //this->image = tmp_size;
    initialize_coordinate_system(orientation_mode);
    LOG4CXX_DEBUG(this->logger,"CALCULATING HISTOGRAMS");
    calculate_angled_line_histogram(orientation_mode);
    if (smoothing_factor > 0)
      smooth_line_histogram(orientation_mode,smoothing_factor);
    calculate_histogram_derivate(derivate_window_size);
    this->image=this->global_image;
  ...
  void Line_Histogram::calculate_angled_line_histogram(string mode){
    reset_histograms_lists();
		initialize_line_histogram(mode);

		for (int r=0; r < this->image.rows; r++)
    {
      int c = this->image.cols/2; //ALSO HALF THE LENGTH OF THE REC
      float angle = this->angles.at<float>(r,c);
      cv::Point P1(r,c);
      cv::Point P2;
      P2.x =  (int)round(P1.x + c * cos(angle * CV_PI / 180.0));
      P2.y =  (int)round(P1.y + c * sin(angle * CV_PI / 180.0));
      cv::Point P3;
      P3.x =  (int)round(P1.x - c * cos(angle * CV_PI / 180.0));
      P3.y =  (int)round(P1.y - c * sin(angle * CV_PI / 180.0));

      clipLine(this->interest_zone,P2,P3);
      cv::LineIterator it(this->image,P2,P3,8);
      for(int i=0; i < it.count; i++ , ++it)
      {
        update_histogram(mode,r,c,this->image.at<uchar>(it.pos()));
      }
		}
		perform_normalization();
  }
